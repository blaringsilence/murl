...
#	2. authority* (username*, password*, host, port) (must begin with // and end either with end of url or / or ? or #)
#	6. path (must begin with / if authority was present, may begin with / if not, but never //)
# 	7. query* (must begin with ?, and is delimited with & or ;)
#	8. fragment* (must begin with #)
# *optional, otherwise = ''

def _getPartEnd(url, possible_ends=['/', '?', '#']):
# Get the a segment of the url, given the url starting from its possible location to the earliest of the ends
...
# Divide the authority part of a URL
	...
	if '@' in authority: # There is an auth part consisting of a username and password
		u_pass = authority.split('@')[0].split(':')
		try:
			username = u_pass[0]
			password = u_pass[1]
		except IndexError as e:
			raise ValueError('There should be a username and a password, but there isn\'t.')
		atPlace = authority.find('@')
		authority = authority[atPlace+1:]

	if authority.startswith(':'):
		raise ValueError('There is no host specified despite there being an authority part,'
						+ 'or there\'s a colon at the beginning of the authority without there being an @ for the authentication.')
	else:
		if ('[' in authority and not ']' in authority) or (']' in authority and not '[' in authority):
		...
		if colonPlace == -1:
			...
			if port < 0 or port > 65536:
				raise ValueError('Port number should be between 0 and 65536 inclusive.')
		elif authority.count(':') > 1:
			raise ValueError('Too many colons in this host, maybe you forgot an @?')
...
# otherwise, will try to divide into single keyvvalue pair, otherwise will present as is.
	qSplit = [querystring]
...
def divideURL(url): 
	...
	return dict(scheme=scheme,authority=authority,path=path,query=query,fragment=fragment)
...
# Re-assemble a URL dict divided by this same module (or uses same syntax)
	...
	if urldict['authority'] != '':
		res += ('//' 
			+ _ifNotEmptyAdd(urldict['authority']['username'], urldict['authority']['username'] + ':' + urldict['authority']['password'] + '@')
			+ _ifNotEmptyAdd(urldict['authority']['host'], urldict['authority']['host'])
			+ _ifNotEmptyAdd(urldict['authority']['port'], ':' + str(urldict['authority']['port']))
			)
	res += _ifNotEmptyAdd(urldict['path'], _assemblePath(urldict['authority'], urldict['path']))
	res += _ifNotEmptyAdd(urldict['query'], '?' + _assembleQuery(urldict['query'], queryDelim))
